...
const DevProject = (props) => {
  ...
  if(props.image != null){
    if(windowSize < 576){
        image = <img className={`project-image img-fluid ${props.slug}`} src={props.image.sm} alt={`${props.name}`}/>
    } else if (windowSize < 768) {
        image = <img className={`project-image img-fluid ${props.slug}`} src={props.image.lg} alt={`${props.name}`}/>
    } else {
        image = <img className={`project-image img-fluid ${props.slug}`} src={props.image.hd} alt={`${props.name}`}/>
    }
  } else {
